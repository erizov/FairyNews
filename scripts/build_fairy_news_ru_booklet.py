...
def _add_lite_defense_section(doc: Any) -> None:
    ...
    doc.add_paragraph(
        "RAG по корпусу сказок не закрывает всю теорию сюжета. Решение — "
        "дополнительная опора на локально подключаемые тексты: в том числе "
        "работы Е. Шварца и классику по структуре волшебной сказки В. Я. "
        "Проппа (промпты, пресеты, пояснения на слайдах; не только выдача "
        "поиска по индексу).",
        style="List Bullet",
    )
    doc.add_paragraph(
